...
namespace Helios.Data
{
    ...
    public class HeliosObjectContext : DbContext, IDbContext
    {
        private static readonly ILog Logger = LogManager.GetLogger<HeliosObjectContext>();

        #region Ctor

        public HeliosObjectContext(string nameOrConnectionString)
...
//                    .Where(type => type.BaseType.GetGenericTypeDefinition() == typeof(HeliosEntityTypeConfiguration<>));
                ...
                {
                    Logger.Debug($"配置对象关系映射({type.FullName})");
